...
//
// UPDATE v1.1.3
// - RETIREMENT_VA now accepts optional exact serviceMonths.
// - serviceMonths is forwarded only to official-retirement.js.
// - Existing yearsOfService/YOS validation remains unchanged.
// - No BAH, PCS_SNAPSHOT, FAD, ASK_ELENA, AIOU, or generic behavior changed.
// ============================================================
...
const BRAIN_VERSION = "thewing-open-brain-1.1.3";
...
function validateRetirementVAInput(input = {}) {
  if (!hasRankInput(input)) {
    throw new Error(
      "Missing rank/paygrade for RETIREMENT_VA calculation."
    );
  }

  if (!hasYosInput(input)) {
    throw new Error(
      "Missing years of service for RETIREMENT_VA calculation."
    );
  }
  ...
  if (
    !Number.isFinite(yosNumber) ||
    yosNumber < 0
  ) {
    throw new Error(
      "Invalid years of service for RETIREMENT_VA calculation."
    );
  }
  ...
  if (!hasVaRatingInput(input)) {
    throw new Error(
      "Missing VA disability rating for RETIREMENT_VA calculation."
    );
  }
  ...
  if (
    !SUPPORTED_VA_RATINGS.includes(
      vaRating
    )
  ) {
    throw new Error(
      `Invalid VA disability rating for RETIREMENT_VA calculation. ` +
      `Supported ratings: ${SUPPORTED_VA_RATINGS.join(", ")}.`
    );
  }
  ...
  if (
    ...
    throw new Error(
      "spouse must be a boolean-compatible value for RETIREMENT_VA calculation."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "dependentParents cannot exceed 2 for RETIREMENT_VA calculation."
    );
  }
}
...
function validatePcsSnapshotInput(input = {}) {
  if (!hasRankInput(input)) {
    throw new Error(
      "Missing rank/paygrade for PCS_SNAPSHOT compensation calculation."
    );
  }

  if (!hasYosInput(input)) {
    throw new Error(
      "Missing years of service for PCS_SNAPSHOT compensation calculation."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Invalid years of service for PCS_SNAPSHOT compensation calculation."
    );
  }

  if (!hasBaseInput(input)) {
    throw new Error(
      "Missing base/duty station for PCS_SNAPSHOT BAH calculation."
    );
  }

  if (!hasDependentInput(input)) {
    throw new Error(
      "Missing dependent status/family value for PCS_SNAPSHOT BAH calculation."
    );
  }
}
...
function normalizeDependents(
  ...
  if (
    options.requireExplicit &&
    !hasExplicit &&
    !hasDependentInput(input)
  ) {
    throw new Error(
      "Missing dependent status/family value for compensation calculation."
    );
  }
...
function sourceVersions() {
  return {
    ...
    payVersion:
      OFFICIAL_PAY_RATE_VERSION ||
      null,

    bahVersion:
      OFFICIAL_BAH_RATE_VERSION ||
      null,

    vaVersion:
      OFFICIAL_VA_RATE_VERSION ||
      null,

    retirementVersion:
      OFFICIAL_RETIREMENT_RATE_VERSION ||
      null
  };
}
...
function buildCanonicalProfile(input = {}, options = {}) {
  ...
  if (
    ...
    throw new Error(
      "Missing rank/paygrade for compensation calculation."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Missing years of service for compensation calculation."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Invalid years of service for compensation calculation."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Missing base/duty station for compensation calculation."
    );
  }
...
function getCompensationProfile(profile) {
  if (
    ...
    throw new Error(
      "BAH calculation failed: base/duty station is required."
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Base Pay calculation failed from official-pay.js for rank ` +
      `${profile.rank || "UNKNOWN"} at ` +
      `${profile.yearsOfService ?? "UNKNOWN"} years of service: ` +
      `${err?.message || "Unknown pay error"}`
    );
  }
  ...
  try {
    ...
    throw new Error(
      `BAH calculation failed from official-bah.js for base ` +
      `${profile.currentBase || "UNKNOWN"}, rank ` +
      `${profile.rank || "UNKNOWN"}, dependents ` +
      `${profile.dependents || "UNKNOWN"}: ` +
      `${err?.message || "Unknown BAH error"}`
    );
  }
  ...
  return {
    ...
    sourceVersion:
      `${OFFICIAL_PAY_RATE_VERSION}+${OFFICIAL_BAH_RATE_VERSION}`
  };
}
...
function getOfficialVACompensation(input) {
  if (
    typeof OFFICIAL_VA.safeGetVACompensation ===
    "function"
  ) {
    const result =
      OFFICIAL_VA.safeGetVACompensation(
        input
      );
    ...
    if (
      ...
      throw new Error(
        result.error ||
        "VA compensation calculation failed."
      );
    }
  ...
  if (
    typeof OFFICIAL_VA.getVACompensation ===
    "function"
  ) {
    return OFFICIAL_VA.getVACompensation(
      input
    );
  }

  throw new Error(
    "official-va.js must export getVACompensation or safeGetVACompensation."
  );
}

function getOfficialRetirementPay(input) {
  if (
    typeof OFFICIAL_RETIREMENT.safeGetRetirementPay ===
    "function"
  ) {
    const result =
      OFFICIAL_RETIREMENT.safeGetRetirementPay(
        input
      );
  ...
  if (
    typeof OFFICIAL_RETIREMENT.getRetirementPay ===
    "function"
  ) {
    return OFFICIAL_RETIREMENT.getRetirementPay(
      input
    );
  }

  throw new Error(
    "official-retirement.js must export getRetirementPay or safeGetRetirementPay."
  );
}
...
function buildRetirementVAProfile(input = {}) {
  ...
  /*
    Optional exact creditable-service months.

    IMPORTANT:
    -------------------------------------------------------------
    - RETIREMENT_VA still requires YOS exactly as before.
    - serviceMonths is supplemental precision for official-retirement.js.
    - Existing callers that do not send serviceMonths behave exactly as before.
  */

  const serviceMonthsRaw =
    input.serviceMonths ??
    input.service_months;
  ...
  if (
    ...
    if (
      ...
      throw new Error(
        "serviceMonths must be a non-negative whole number for RETIREMENT_VA calculation."
      );
    }
...
function resolveRetirementAccuracy(retirementRecord = {}) {
  ...
  if (
    ...
    return {
      ...
      compensationAccuracy:
        "official_va_and_high36_retirement",
  ...
  return {
    ...
    compensationAccuracy:
      "official_va_and_retirement_estimate",

    retirementLabel:
      "Retirement estimate using final monthly basic pay proxy"
  };
}

function buildRetirementVAPayload(
  input = {},
  tool = "RETIREMENT_VA"
) {
  validateRetirementVAInput(
    input
  );

  const profile =
    buildRetirementVAProfile(
      input
    );
  ...
  /*
    Forward exact service months only when the RETIREMENT_VA
    caller supplied them.

    This is intentionally isolated to the retirement branch.

    BAH, PCS_SNAPSHOT, FAD, ASK_ELENA, AIOU and generic
    compensation behavior are untouched.
  */

  if (
    profile.serviceMonths !==
    null
  ) {
    retirementInput.serviceMonths =
      profile.serviceMonths;
  }
  ...
  if (
    ...
    input.high36MonthlyArray.length >
      0
  ) {
    retirementInput.high36MonthlyArray =
      input.high36MonthlyArray;
  }

  const retirementRecord =
    getOfficialRetirementPay(
      retirementInput
    );

  const accuracy =
    resolveRetirementAccuracy(
      retirementRecord
    );
  ...
  const compensation = {
    ...
    detail: {
      ...
      retirementBaseMethod:
        accuracy.retirementBaseMethod,

      compensationAccuracy:
        accuracy.compensationAccuracy,

      retirementLabel:
        accuracy.retirementLabel,

      sourceModules
    },

    sourceVersion:
      `${OFFICIAL_PAY_RATE_VERSION}+${OFFICIAL_RETIREMENT_RATE_VERSION}+${OFFICIAL_VA_RATE_VERSION}`
  };
...
function buildSummaryFromComp(
  ...
  return {
    ...
    headline:
      `Estimated monthly active-duty compensation is $${combinedMonthlyGross.toLocaleString()}.`,
...
function buildGenericPayload(
  ...
  const bahRecord =
    compensation?.detail?.bahRecord ||
    {};

  const payRecord =
    compensation?.detail?.payRecord ||
    {};
  ...
  return {
    ...
    summary:
      buildSummaryFromComp(
        profile,
        compensation
      ),

    housing:
      buildHousingBaseline(
        profile,
        compensation
      ),
    ...
    pay: {
      ...
      payAccuracy:
        "deterministic_official_modules",
...
function buildPcsSnapshotPayload(
  profile,
  compensation
) {
...
function buildAskElenaPayload(
  ...
  return {
    ...
    askElena: {
      ...
      residual:
        payload.readiness.residual,

      housingSafeTarget:
        payload.housing
          .safeHousingTarget,

      housingStretchTarget:
        payload.housing
          .stretchHousingTarget
    }
  };
...
function buildPayload(
  ...
  if (
    ...
    return buildRetirementVAPayload(
      input,
      "RETIREMENT_VA"
    );
  }
  ...
  if (
    ...
    return buildPcsSnapshotPayload(
      profile,
      compensation
    );
  }
    ...
    if (
      ...
      return json(
        200,
        {
          ...
          purpose:
            "Public calculator brain for PCSUnited tools powered by TheWing.ai.",
