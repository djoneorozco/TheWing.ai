// netlify/functions/_share/pt-scoring-core.js
// ============================================================
// TheWing.ai • USAF PFRA / PT Scoring Core (2026)
// Platform-neutral deterministic scoring extracted from the
// audited PT Calculator (cursor/fix-pt-calculator-scoring-bd8f).
//
// NO Netlify handler / OpenAI / Supabase / DOM / network.
// ============================================================

export const PT_SCORING_CORE_VERSION = "pt-scoring-core-2026.1";

export const PT_SOURCE = Object.freeze({
  manual: "AFMAN 36-2905",
  manual_date: "2026-03-24",
  scoring_effective_date: "2026-03-01",
  implementation_date: "2026-07-01",
  manual_path: "/doc/dafman36-2905.pdf",
  scoring_chart_path: "/doc/PFRA Scoring Charts.pdf"
});
...
export function roundWHtR(ratio) {
  return Math.round(Number(ratio) * 100) / 100;
}

export function getWHtRScore(ratio) {
...
export function bodyCompositionPassed(ratio) {
  return roundWHtR(ratio) <= MIN_PASS.bodyMaxRatio;
}

export function scoreHigherBetter(value, rows, key) {
...
export function scoreTimeLowerBetter(sec, rows, key) {
  if (!rows || !rows.length) return null;
  for (const [pts, map] of rows) {
    const threshold = map?.[key];
    if (!Number.isFinite(threshold)) continue;
...
export function scoreWalk(sec, key) {
...
export function scoreFromTable(table, key, value, direction) {
...
export function scoreCategory(total, minimumsMet) {
...
export function componentMinimumsMet(flags) {
  return Boolean(
    flags.bodyPassed &&
...
export function pairKeyFromSexAge(sex, ageBandKey) {
  const sexKey = sex === "female" ? "female" : sex === "male" ? "male" : null;
  if (!sexKey || !ageBandKey) return null;
  return `${ageBandKey}_${sexKey}`;
}

export function getStrengthTable(option) {
  if (option === "hand_release_push_ups" || option === "hrpu") return TABLES.hrpu;
  if (option === "push_ups" || option === "push") return TABLES.push;
  return null;
}

export function getCoreTable(option) {
  if (option === "cross_leg_reverse_crunch" || option === "crunch" || option === "rev_crunch") return TABLES.crunch;
  if (option === "forearm_plank" || option === "plank") return TABLES.plank;
  if (option === "sit_ups" || option === "situp" || option === "sit_up") return TABLES.situp;
  return null;
}
